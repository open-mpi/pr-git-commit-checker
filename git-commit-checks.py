...
3. If required (by the config inputs via env variables), ensure that a
...
# Because we can't have true Boolean env variables, we need to convert them from
# str to bool. We know that the values will be YAML bools (true or false, not 1
# or 0), so we can just do this simple conversion. If the string is "true", than
# the expression evaluates to bool True. If the string is "false", then the
# expression evaluates to bool False.
CHERRY_PICK_REQUIRED = os.environ.get('CHERRY_PICK_REQUIRED').lower() == "true"
PERMIT_EMPTY         = os.environ.get('PERMIT_EMPTY').lower() == "true"
...
def load_config():
    # Defaults
    config = {
        'cherry pick required' : CHERRY_PICK_REQUIRED,
        'permit empty' : PERMIT_EMPTY,
        'bad emails' : [
            '^root@',
            'localhost',
            'localdomain',
